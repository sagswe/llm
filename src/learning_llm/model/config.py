...
class ModelConfig:
    ...
    Tiny defaults keep examples inexpensive on a CPU. Tensor dimensions use the
    repository notation: batch ``B``, sequence length ``T``, model width
    ``C = d_model``, number of heads ``H = n_head``, and per-head width
    ``D = C / H``.
    """
    ...
    def validate_sequence_length(self, sequence_length: int) -> None:
        """Reject a requested sequence length ``T`` beyond the context window."""
    ...
    def gpt2_small(cls) -> "ModelConfig":
        ...
        is too large for the repository's CPU-friendly exercises.
        """
        return cls(
            vocab_size=50_257,
